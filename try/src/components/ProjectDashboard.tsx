...
import { useProject, ProjectMetadata } from "@/contexts/ProjectContext";
...
export default function ProjectDashboard() {
  ...
  // Load query intents when current project changes
  useEffect(() => {
    const loadIntents = async () => {
      if (currentProject?.id) {
        setIsLoadingIntents(true);
        try {
          const intents = await getQueryIntents(currentProject.id, 10);
  ...
  const getDialectColor = (dialect: string) => {
    ...
    return (
      colors[dialect as keyof typeof colors] || "bg-gray-100 text-gray-800"
    );
  };
        ...
        <div className="max-w-4xl mx-auto">
          <div className="mb-8">
            ...
            <p className="text-gray-600 mt-2">
              Set up your SQL workspace with intelligent AI assistance
            </p>
          </div>
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <div className="flex items-center gap-3 mt-1">
                    <span
                      className={`px-2 py-1 rounded-full text-xs font-medium ${getDialectColor(
                  ...
                  <div className="space-y-4">
                    <div className="flex justify-between items-center">
                      ...
                      <span className="font-semibold">
                        {currentProject.createdTables.length}
                      </span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span className="text-sm text-gray-600">
                      ...
                      <span className="font-semibold">
                        {isLoadingIntents ? "..." : queryIntents?.length || 0}
                      </span>
                    </div>
                    <div className="flex justify-between items-center">
                    ...
                    <div className="flex justify-between items-center">
                      ...
                      <span className="text-sm">
                        {formatDistanceToNow(
                          new Date(currentProject.updatedAt)
                        )}{" "}
                        ago
                      </span>
                    </div>
            ...
            <div className="lg:col-span-2 space-y-6">
                ...
                <CardContent>
                  {currentProject.createdTables.length === 0 ? (
                    <div className="text-center py-8 text-gray-500">
                      ...
                      <p className="text-sm">
                        Start by generating a schema or creating tables manually
                      </p>
                    </div>
                  ) : (
                    <div className="space-y-3">
                      {currentProject.createdTables
                        .slice(-3)
                        .map((table, index) => (
                          <div
                            key={index}
                          ...
                          >
                            <div>
                              ...
                              <p className="text-sm text-gray-500">
                                Created{" "}
                                {formatDistanceToNow(new Date(table.createdAt))}{" "}
                                ago
                              </p>
                            </div>
              ...
              <Card>
                ...
                <CardContent>
                  ...
                  ) : !queryIntents || queryIntents.length === 0 ? (
                    <div className="text-center py-8 text-gray-500">
                      <History className="h-12 w-12 mx-auto mb-4 opacity-50" />
                      <p>No queries executed yet</p>
                      <p className="text-sm">
                        Use the AI Assistant or SQL Editor to run your first
                        query
                      </p>
                    </div>
                  ) : (
                          ...
                          >
                            <div className="flex-1">
                              <code className="text-sm bg-white px-2 py-1 rounded border">
                                {query.sqlQuery.slice(0, 50)}
                                {query.sqlQuery.length > 50 ? "..." : ""}
                              </code>
                              {query.userQuestion && (
                                <p className="text-sm text-gray-600 mt-1 italic">
                                  &ldquo;{query.userQuestion}&rdquo;
                                </p>
                              )}
                              <p className="text-xs text-gray-500 mt-1">
                                {formatDistanceToNow(
                                  new Date(query.executedAt)
                                )}{" "}
                                ago •{" "}
                                {query.wasSuccessful ? "✓ Success" : "✗ Failed"}
      ...
      <div className="max-w-6xl mx-auto">
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-gray-600 mt-2">
              Create and manage your database projects with AI-powered
              assistance
            </p>
          </div>
          <Button
          ...
          <Card className="text-center py-12">
            <CardContent>
              ...
              <CardDescription className="mb-6">
                Create your first SQL project to start generating queries with
                AI assistance
              </CardDescription>
              <Button
                onClick={() => setShowCreateForm(true)}
